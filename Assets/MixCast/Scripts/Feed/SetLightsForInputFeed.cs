...
namespace BlueprintReality.MixCast {
	public class SetLightsForInputFeed : MonoBehaviour {
        ...
        bool LightIsAffectingPlayer(Light l, float playerDepth)
        {
            switch(l.type)
            {
                ...
                case LightType.Point:
                    float playerDistance = Mathf.Lerp(feed.cam.gameCamera.nearClipPlane, feed.cam.gameCamera.farClipPlane, playerDepth);
                    float dot = Vector3.Dot(feed.cam.gameCamera.transform.forward, l.transform.position - feed.cam.gameCamera.transform.position);
                    if (Mathf.Abs(dot - playerDistance) > l.range)
                        return false;
                    if (playerDistance < dot && !includeBacklighting)
                        return false;
                    return true;
                ...
                case LightType.Spot:
                    ...
                    float playerDistance2 = Mathf.Lerp(feed.cam.gameCamera.nearClipPlane, feed.cam.gameCamera.farClipPlane, playerDepth);
                    float dot2 = Vector3.Dot(feed.cam.gameCamera.transform.forward, l.transform.position - feed.cam.gameCamera.transform.position);
                    if (Mathf.Abs(dot2 - playerDistance2) > coneSideLength)
                        return false;
                    if (playerDistance2 < dot2 && !includeBacklighting)
                        return false;
                    return true;
