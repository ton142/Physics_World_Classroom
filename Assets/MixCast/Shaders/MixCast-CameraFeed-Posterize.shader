...
Shader "MixCast/Camera Feed (Posterize)" {
...
#include "Includes/MixCastLighting.cginc"
	...
	frag_output frag(v2f i) {
		...
		float3 worldPos = CalculateWorldPosition(float3(i.uv.x, i.uv.y, _PlayerDepth), _CamNear, _CamFar, _CamProjection, _CamToWorld);
		...
		inputRGB = pow(inputRGB, _PosterizeGamma);
		inputRGB *= _PosterizeLevels;
		inputRGB = floor(inputRGB);
		inputRGB /= _PosterizeLevels;
		inputRGB = pow(inputRGB, 1.0 / _PosterizeGamma);

		//Assemble final output
		o.col = float4(2.0 * _Color.rgb * inputRGB, outputAlpha * _Color.a);

#ifdef DEPTH_ON
		o.dep = Buffer01Depth(_PlayerDepth);
#endif
