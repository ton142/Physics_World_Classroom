...
//Chroma
float CalculateChromaAlpha(float3 inputRGB, float3 keyHSV, float3 channelLimits, float3 channelFeathers, float3 channelFactors)
{
...
void ApplyBackgroundRemoval(float3 inputRGB, float2 inputUvs, inout float inputAlpha)
{
#ifdef BG_REMOVAL_CHROMA
	inputAlpha *= CalculateChromaAlpha(inputRGB, _KeyHsvMid, _KeyHsvRange, _KeyHsvFeathering, float3(1, 1, 1));
#elif BG_REMOVAL_STATIC
	float3 keyHsvMid = tex2D(_KeyMidTex, inputUvs).rgb;
	float3 keyHsvRange = tex2D(_KeyRangeTex, inputUvs).rgb;
	inputAlpha *= CalculateChromaAlpha(inputRGB, keyHsvMid, keyHsvRange, _KeyHsvFeathering, float3(1, 1, 1));
#endif
}